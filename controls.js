...
window.PageControls = (() => {
  ...
  function updatePageLabels({
    ...
    pages.forEach((page) => {
      ...
      const isFinalRightPlaceholder =
        isFinalRightPlaceholderPage(pageNumberValue);
      const isLeft = side === "left";
      const canTurn =
        pageExists &&
        (isLeft
          ? currentSpreadIndex > 0
          : currentSpreadIndex < notebookSpreadCount - 1);
      ...
      page.classList.toggle(
        "is-missing-page",
        !pageExists && !isFinalRightPlaceholder,
      );
      ...
      page
        ...
        ?.replaceChildren(pageExists ? pageNumber : "");

      page
        .querySelectorAll("[data-page-fold-number]")
        .forEach((foldElement) => {
          foldElement.textContent = canTurn ? formatPageNumber(foldNumber) : "";
        });
      page
        .querySelectorAll("[data-page-fold-behind-number]")
        .forEach((foldElement) => {
          foldElement.textContent = canTurn
            ? formatPageNumber(behindNumber)
            : "";
        });
  ...
  function normalizeNotebookPageCount({
    pageCount,
    ...
    const roundedPageCount =
      Math.round(Number(pageCount)) || initialNotebookPageCount;
  ...
  function formatPageNumber({ pageNumber, notebookPageCount }) {
    return isPageNumberAvailable({ pageNumber, notebookPageCount })
      ? String(pageNumber)
      : "";
  }
  ...
  function getFocusedPageState({
    ...
    const nextPageNumber = clamp(
      Math.round(Number(pageNumber)) || 0,
      0,
      notebookPageCount - 1,
    );
  ...
  function updatePageActionButtons({
    ...
    const focusedPageExists = isPageNumberAvailable({
    ...
    if (deletePageButton) {
      deletePageButton.disabled =
        notebookPageCount <= minNotebookPageCount || !focusedPageExists;
    }
  ...
  function updateSpreadItemVisibility({
    ...
    setSelectedItem(
      selectedItems.size ? Array.from(selectedItems).at(-1) : null,
    );
    updateObjectControlsState();
  }
  ...
  function isCornerPointer(page, event, { gridColumns, gridRows }) {
    ...
    const hitAreaGridCells =
      PlannerRootControls.controls.pageCornerFlip.hitAreaGridCells;
    const isBottomCornerArea =
      localY >= rect.height - cellHeight * hitAreaGridCells;

    if (page.dataset.turnPage === "next") {
      return (
        isBottomCornerArea &&
        localX >= rect.width - cellWidth * hitAreaGridCells
      );
    }
